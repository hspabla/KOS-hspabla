...
void* AcpiOsMapMemory(ACPI_PHYSICAL_ADDRESS Where, ACPI_SIZE Length) {
  ...
  paddr pma = align_down(paddr(Where), smallps);
  size_t size = align_up(paddr(Where) + Length, smallps) - pma;
  vaddr vma = kernelAS.mmap<smallpl,false>( 0, size, pma );
...
void AcpiOsUnmapMemory(void* LogicalAddress, ACPI_SIZE Size) {
  ...
  vaddr vma = align_down(vaddr(LogicalAddress), smallps);
  size_t size = align_up(Size + vaddr(LogicalAddress) - vma, smallps);
  kernelAS.munmap<smallpl,false>( vma, size );
